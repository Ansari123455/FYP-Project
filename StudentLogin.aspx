...
<head id="Head1" runat="server">
    ...
    <title>FYP Portal| Student Login !!</title>
...
<script type="text/javascript">
    function PasteTextInEditor() {
        var result = "UOS/";
        result += document.getElementById("<%=dd1.ClientID%>").options[document.getElementById("<%=dd1.ClientID%>").selectedIndex].text;
        result += "-";
        result += document.getElementById("<%=dd2.ClientID%>").options[document.getElementById("<%=dd2.ClientID%>").selectedIndex].text;
        result += "-";
        result += document.getElementById("<%=TextBox1.ClientID %>").value;
        result += "/LHR";

        document.getElementById("<%=txtusername.ClientID%>").value = result;
   ...
   <nav role='navigation'>
    ...
    <form id="login" runat="server" autocomplete="off">
        ...
        Login to your account!</h1>
            <div>  
            <asp:Label ID="Label7" runat="server" Text="" color="green"></asp:Label>
            <br />  
            <label for="login_username">Student ID:*</label>
            <asp:DropDownList ID="dd1" runat="server" onchange="PasteTextInEditor()">
            </asp:DropDownList>
            <asp:DropDownList ID="dd2" runat="server" onchange="PasteTextInEditor()">
            </asp:DropDownList>
            <asp:TextBox ID="TextBox1" runat="server" Text="000" width="40px" height="16px" maxlength="20" onkeyup="PasteTextInEditor()"></asp:TextBox>
            <asp:RegularExpressionValidator ID="IntValidatorOrderSalesRecordNumber" runat="server" ValidationGroup="1"
                ErrorMessage="Numeric Values Only!" SetFocusOnError="True" ValidationExpression="(\d+)" ControlToValidate="TextBox1"
                Display="Dynamic">
            </asp:RegularExpressionValidator>
            <asp:TextBox ID="txtusername" runat="server" ReadOnly="true" class="field required" title="Please provide your username?" placeholder="Student ID"/>
            </div>
            <div>
